...
\item 11 of the 12 parameters were allowed to float freely (i.e. no symmetry relations were imposed a priori).
...
\item Convergence criteria were set as a minimum gradient reduction, but also capped by the total number of function evaluations to prevent run-away cases.
\item Results herein are presented are categorized by reduced $\chi^2$ values ($=\chi^2/(N-N_{vars}$, where $N$ is the number of data points, and $N_{vars}$ the number of parameters) unless otherwise noted.
\end{itemize}
