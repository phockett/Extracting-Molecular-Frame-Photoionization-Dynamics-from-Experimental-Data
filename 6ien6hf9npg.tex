...
Following this notation, it is also trivial to write the radial matrix elements in density matrix form:

\mathbb{I}^{\zeta,\zeta'}\mathbf{\rho}^{\zeta\zeta'}


full final state as a density matrix in the $\zeta\zeta'$ representation:

\begin{equation}
\mathbf{\bar{\rho}}_{L,M}^{\zeta\zeta'}=\varUpsilon_{L,M}^{u,\zeta\zeta'}\mathbb{I}^{\zeta,\zeta'}\mathbf{\rho}^{\zeta\zeta'}
\end{equation}
...
the given measurement),measurement), with dimensions dependent on the unique sets of quantum numbers required (in the simplest case, this will just be a set of partial waves $(l,m)$). The $L,M$ notation indicates here that these dimensions should not be summed over, hence the tensor coupling into the $\beta_{L,M}^{u}$ parameters can also be written in terms of the density matrix:
