...
A further simplification can be made, to provide the observables in terms of ``channel functions", which define the ionization continuum for a given set of parameters $u$ (e.g. defined by an experimental configuration), and 
...
This form indicates the equivalence of the $\beta_{L,M}^{u}$ terms to the state multipoles (geometric tensors), which can be thought of as a coupled basis equivalent of the density matrix representations (see, e.g., ref. \cite{BlumDensityMat}, Chpt. 4.).

TODO: confirm notation here, may want to include sum over L,M, as it is implicit later? (See Fig. \ref{998904}, should also confirm representation here from outer product case, maybe plot LM representation too - see https://epsproc.readthedocs.io/en/dev/methods/density_mat_notes_demo_300821.html)
