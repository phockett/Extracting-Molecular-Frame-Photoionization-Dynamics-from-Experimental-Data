...
Within this treatment, the observables can be defined in a series of simplified forms, emphasizing the quantities of interest for a given problem. Some details are defined in the following subsections, and Appendix \ref{appendix:formalism}.

\subsubsection{Channel functions \& density matrix representation}

A simple form of the equations, amenable to fitting, is to write the observables in terms of ``channel functions", which define the ionization continuum for a given set of parameters $u$ (e.g. defined by an experimental configuration),
...
\begin{equation}
\mathbf{\rho}_{l,m}^{\zeta\zeta'} = |\zeta\rangle\langle\zeta'| \equiv \mathbb{I}^{\zeta,\zeta'}
\end{equation}

And the full final state as a density matrix in the $\zeta\zeta'$ representation:

\begin{equation}
\mathbf{\rho}_{l,m}^{\zeta\zeta'}=\varUpsilon_{L,M}^{u,\zeta\zeta'}\mathbb{I}^{\zeta,\zeta'}
\end{equation}
...
the given measurement), with dimensions dependent on the unique sets of quantum numbers required. In the simplest case, this will just be a set of partial waves $\zeta = (l,m)$, which are explicitly labelled as the usual. The $L,M$ notation indicates here that these dimensions should not be summed over, hence the tensor coupling into the $\beta_{L,M}^{u}$ parameters can also be written in terms of the density matrix:
