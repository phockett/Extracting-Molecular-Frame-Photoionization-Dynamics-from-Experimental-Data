...
# PH 07/09/22 - testing more options here.
# For figs avoiding page overflow, try https://tex.stackexchange.com/a/87886, suggests 'height=\dimexpr \textheight - 4\baselineskip\relax'
# THIS WORKS WELL FOR LONG, but NOT WIDE, figs!
# FIGURE_DEFAULTS = {'placement': '', 'options': r'height=\dimexpr \textheight - 4\baselineskip\relax', 'figure_env': 'figure'}
# General safe fig placement from https://tex.stackexchange.com/a/121347 - seems to work well! May want to change to columnwidth?
FIGURE_DEFAULTS = {'placement': '', 'options': r'width=\textwidth,height=\dimexpr\textheight-4\baselineskip-\abovecaptionskip-\belowcaptionskip\relax,keepaspectratio', 'figure_env': 'figure'}
...
#             return f.read()
    ...
    textIn = textIn.replace('<b>',r'\textbf{').replace('</b>',r'}').replace('%','\%')   #.replace('&','\&')   # Replace bold tags & % - should use Pandoc here!
                                                                                       # UGLY - & replace breaks &nbsp;
    textIn = textIn.replace('<i>',r'\textit{').replace('</i>',r'}')
    # TODO better methods and/or use Pandoc
    # textDict = {<i>}
    ...
    # Figure labels from div key
    # None object if no keys, so just lazy try/except here.
    # if "data-label" in soup.div.keys():
    try:
        clean_text = clean_text + f'\\label{{{soup.div["data-label"]}}}'
    except:
        pass
...
def get_figure_string(filename, localdir, inputdir, flatten=False, copyto=False):
    ...
    if copyto:
        ...
        # PH 08/09/22 - TODO, Update figfn path to new dir otherwise always sets to root dir.
        # os.path.join(copyto, os.path.split(figpath)[1])
...
def build_authorea_latex(localdir, builddir, latex_exec, bibtex_exec, outname,
    ...
    with open(os.path.join(localdir, 'layout.md')) as f:
        for l in f:
            ...
            # PH 07/09/22 - added HTML parser.
            elif ls in ('abstract.tex','abstract.html'):   # or n==0:  - treat first section as abstract?
                # add abstract to title content
                if ls.endswith('.html'):
                    titlein = get_input_string_HTML('abstract', get_in_path(localdir, builddir, pathtype), flatten=flatten)
                else:
                ...
                titlecontent.append(r'\tableofcontents')  # PH 08/09/22 Add ToC after abstract, should add a flag for this.
                ...
                # PH 08/09/22 - copy figs to build_dir/figures
                sectioninputs.append(get_figure_string(ls, localdir, inpath, flatten=flatten,
                                                       copyto=os.path.join(builddir,'figures') if copy_figs else False))
