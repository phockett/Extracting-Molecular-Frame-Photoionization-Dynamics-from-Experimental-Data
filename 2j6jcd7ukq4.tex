...
Here $()^{+}$ indicates the M Significantly, the matrix elements are not required for inversion, provided that either $\mathbf{C}^{mol}$ or $\mathbf{C}^{lab}$ is known (e.g. from a measurement), and that the reduced channel functions are also known.
