...
\begin{equation}
C_{PR}^{LM}(\epsilon,\Delta q)=\sum_{\zeta\zeta'}\mathbb{I}_{\zeta\zeta'}(\epsilon)\Gamma_{PR\Delta q}^{\zeta\zeta'LM}
\end{equation}
...
Where $\mathbf{\Gamma}$ are the reduced channel functions, indexed by all required quantum numbers for the LF and MF cases. Again, full details can be found in Appendix \ref{app:mat-inversion}. Given these, the transformation matrix can be written as:
